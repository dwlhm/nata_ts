...
Register-ArgumentCompleter -Native -CommandName 'deno' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'deno' {
            ...
            [CompletionResult]::new('--env-file', '--env-file', [CompletionResultType]::ParameterName, 'Load environment variables from local file   Only the first environment variable with a given key is used.   Existing process environment variables are not overwritten.')
            ...
            [CompletionResult]::new('--unstable-detect-cjs', '--unstable-detect-cjs', [CompletionResultType]::ParameterName, 'Reads the package.json type field in a project to treat .js files as .cjs')
            ...
            [CompletionResult]::new('add', 'add', [CompletionResultType]::ParameterValue, 'Add dependencies to your configuration file.   deno add jsr:@std/path  You can add multiple dependencies at once:   deno add jsr:@std/path jsr:@std/assert')
            ...
            [CompletionResult]::new('task', 'task', [CompletionResultType]::ParameterValue, 'Run a task defined in the configuration file.   deno task build  List all available tasks:   deno task')
        ...
        'deno;compile' {
            ...
            [CompletionResult]::new('--strace-ops', '--strace-ops', [CompletionResultType]::ParameterName, 'Trace low-level op calls')
            [CompletionResult]::new('--check', '--check', [CompletionResultType]::ParameterName, 'Set type-checking behavior. This subcommand type-checks local modules by default, so adding --check is redundant   If the value of "all" is supplied, remote modules will be included.   Alternatively, the ''deno check'' subcommand can be used')
            [CompletionResult]::new('--include', '--include', [CompletionResultType]::ParameterName, 'Includes an additional module in the compiled executable''s module graph.   Use this flag if a dynamically imported module or a web worker main module   fails to load in the executable. This flag can be passed multiple times,   to include multiple additional modules.')
